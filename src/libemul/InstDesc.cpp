...
/** Lease / Release ***/
InstDesc *emulLease(InstDesc *inst, ThreadContext *context){
	Pid_t pid = context->getPid();
	//The number of addresses we would like to group together and lease
	uint32_t numAddrs = ArchDefs<ExecModeMips32>::getReg<uint32_t,RegTypeGpr>(context,ArchDefs<ExecModeMips32>::RegA0);	
	//Lease time is in microseconds
	uint32_t leaseTime = ArchDefs<ExecModeMips32>::getReg<uint32_t,RegTypeGpr>(context,ArchDefs<ExecModeMips32>::RegA1);
	//point to the array of addresses to lease on the stack
	uint32_t lAddr = ArchDefs<ExecModeMips32>::getReg<uint32_t,RegTypeGpr>(context,ArchDefs<ExecModeMips32>::RegA2);
	printf("Leasing\n");
	printf("Number, %d\n", numAddrs);
	printf("Lease Time, %d\n", leaseTime);
	printf("arrayadd, %d\n", lAddr);
...
InstDesc * emulRelease(InstDesc *inst, ThreadContext *context){
	Pid_t pid = context->getPid();
	//point to the addresse to release
	uint32_t lAddr = ArchDefs<ExecModeMips32>::getReg<uint32_t,RegTypeGpr>(context,ArchDefs<ExecModeMips32>::RegA0);
	printf("release\n");	
	printf("addr, %d\n", lAddr);
...
#endif
	//if lease
	ops[OpKey(0xFC00003F,0xbc000000)]<<OpData( "lease"    , CtlNorm , TypLease   , ArgNo  , ArgNo  , ArgNo  , ImmNo  ,emulLease);
    ops[OpKey(0xFC00003F,0xbc000001)]<<OpData( "release"      , CtlNorm , TypRelease   , ArgNo  , ArgNo  , ArgNo  , ImmNo  , emulRelease);
	//end lease
