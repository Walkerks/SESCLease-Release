...
void MESIProtocol::preInvalidate(Line *l)
{
    ...
    if(l->isDirty())
		//if the block is dirty we need time to evict and write
        changeState(l, MESI_TRANS_INV_D);
    else
		//Otherwise we just need time to evict
...
void MESIProtocol::read(MemRequest *mreq)
{
	...
	//Mark the block as being currently allocated
